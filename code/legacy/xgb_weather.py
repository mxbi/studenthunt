...
vidp = pd.read_csv('../weather/VABB.csv')

vidp['Date'] = vidp['IST'].apply(lambda x: x.split('-')[2] + '-' + x.split('-')[1] + '-' + x.split('-')[0])
df_train = df_train.merge(vidp, on='Date', how='left')
df_test = df_test.merge(vidp, on='Date', how='left')
...
drop_cols = ['Date', 'year', 'date_dm', 'dateidix', ' Events', 'IST']
#########################################
...
if validation:
    y_train = df_train.loc[df_train.year <= 1999].loc[df_train.year >= 1996]['Footfall']
    x_train = df_train.loc[df_train.year <= 1999].loc[df_train.year >= 1996].drop(['ID', 'Footfall'] + drop_cols, 1)
else:
    print('VALIDATION OFF')
    y_train = df_train['Footfall']
    x_train = df_train.drop(['ID', 'Footfall'] + drop_cols, 1)

y_valid = df_train.loc[df_train.year > 1999]['Footfall']
x_valid = df_train.loc[df_train.year > 1999].drop(['ID', 'Footfall'] + drop_cols, 1)
...
clf = xgb.train(params, d_train, 2000, watchlist, early_stopping_rounds=500, verbose_eval=25)
