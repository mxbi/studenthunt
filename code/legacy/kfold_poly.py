...
for fold in range(10):
    ...
    mx_train, _, my_train, _ = train_test_split(x_train, y_train, train_size=0.9, random_state=fold)

    d_train = xgb.DMatrix(mx_train, label=my_train)
    ...
    clf = xgb.train(params, d_train, 10, watchlist, early_stopping_rounds=50, verbose_eval=1)
...
ps = pd.DataFrame(ps)
print(ps)
p_test = ps.mean(axis=1)

sub = pd.DataFrame()
sub['ID'] = id_test
sub['Footfall'] = p_test
sub.to_csv('kfold_poly.csv', index=False)
